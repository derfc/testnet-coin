module testnet_coins::aptos_launch_token {
    ...
    struct AptosLaunchToken has key, store { }
    ...
    public entry fun initialize(coin_admin: &signer) {
        ...
        let (burn_cap, freeze_cap, mint_cap) = coin::initialize<AptosLaunchToken>(
            coin_admin,
            string::utf8(b"Aptos Launch Token"),
            string::utf8(b"ALT"),
            8, /* decimals */
            true, /* monitor_supply */
        );

        move_to(coin_admin, CapStore<BurnCapability<AptosLaunchToken>> { cap: burn_cap });
        move_to(coin_admin, CapStore<FreezeCapability<AptosLaunchToken>> { cap: freeze_cap });
        move_to(coin_admin, CapStore<MintCapability<AptosLaunchToken>> { cap: mint_cap });
        move_to(coin_admin, Delegations<BurnCapability<AptosLaunchToken>>{ inner: vector::empty<DelegatedCapability<BurnCapability<AptosLaunchToken>>>() });
        move_to(coin_admin, Delegations<FreezeCapability<AptosLaunchToken>>{ inner: vector::empty<DelegatedCapability<FreezeCapability<AptosLaunchToken>>>() });
        move_to(coin_admin, Delegations<MintCapability<AptosLaunchToken>>{ inner: vector::empty<DelegatedCapability<MintCapability<AptosLaunchToken>>>() });
    ...
    public fun destroy_burn_cap_store(cap_store: CapStore<BurnCapability<AptosLaunchToken>>) {
        let CapStore<BurnCapability<AptosLaunchToken>> { cap: cap } = cap_store;
        coin::destroy_burn_cap(cap);
    }
    public fun destroy_freeze_cap_store(cap_store: CapStore<FreezeCapability<AptosLaunchToken>>) {
        let CapStore<FreezeCapability<AptosLaunchToken>> { cap: cap } = cap_store;
        coin::destroy_freeze_cap(cap);
    }
    public fun destroy_mint_cap_store(cap_store: CapStore<MintCapability<AptosLaunchToken>>) {
        let CapStore<MintCapability<AptosLaunchToken>> { cap: cap } = cap_store;
    ...
    public entry fun revoke_burn_capability(sender: &signer, account: address) acquires AdminStore, CapStore {
        ...
        assert!(exists<CapStore<BurnCapability<AptosLaunchToken>>>(account), error::not_found(ENO_CAPABILITIES));
        destroy_burn_cap_store(move_from<CapStore<BurnCapability<AptosLaunchToken>>>(account));
    }
    public entry fun revoke_freeze_capability(sender: &signer, account: address) acquires AdminStore, CapStore {
        only_admin_or_self(sender, account);
        assert!(exists<CapStore<FreezeCapability<AptosLaunchToken>>>(account), error::not_found(ENO_CAPABILITIES));
        destroy_freeze_cap_store(move_from<CapStore<FreezeCapability<AptosLaunchToken>>>(account));
    }
    public entry fun revoke_mint_capability(sender: &signer, account: address) acquires AdminStore, CapStore {
        only_admin_or_self(sender, account);
        assert!(exists<CapStore<MintCapability<AptosLaunchToken>>>(account), error::not_found(ENO_CAPABILITIES));
        destroy_mint_cap_store(move_from<CapStore<MintCapability<AptosLaunchToken>>>(account));
    }
    ...
    // privileged function
    public fun burn(
        coin: Coin<AptosLaunchToken>,
        cap_store: &CapStore<BurnCapability<AptosLaunchToken>>
    )   {
        let burn_cap = &cap_store.cap;
        coin::burn<AptosLaunchToken>(coin, burn_cap);
    }

    public fun freeze_account(
        account_to_freeze: address,
        cap_store: &CapStore<FreezeCapability<AptosLaunchToken>>
    )   {
        let freeze_cap = &cap_store.cap;
        coin::freeze_coin_store<AptosLaunchToken>(account_to_freeze, freeze_cap);
    }

    public fun mint(
        amount: u64,
        cap_store: &CapStore<MintCapability<AptosLaunchToken>>
    ): Coin<AptosLaunchToken>   {
        let mint_cap = &cap_store.cap;
        coin::mint<AptosLaunchToken>(amount, mint_cap)
    }
    ...
    public entry fun burn_entry(
        ...
        let account_addr = signer::address_of(account);
        assert!(exists<CapStore<BurnCapability<AptosLaunchToken>>>(account_addr), error::not_found(ENO_CAPABILITIES));
        let burn_cap = &borrow_global<CapStore<BurnCapability<AptosLaunchToken>>>(account_addr).cap;
        coin::burn_from<AptosLaunchToken>(account_addr, amount, burn_cap);
    }
    ...
    public entry fun freeze_entry(
        ...
        let account_addr = signer::address_of(account);
        assert!(exists<CapStore<FreezeCapability<AptosLaunchToken>>>(account_addr), error::not_found(ENO_CAPABILITIES));
        let freeze_cap = &borrow_global<CapStore<FreezeCapability<AptosLaunchToken>>>(account_addr).cap;
        coin::freeze_coin_store<AptosLaunchToken>(account_to_freeze, freeze_cap);
    }
    ...
    public entry fun mint_entry(
        ...
        let account_addr = signer::address_of(account);
        assert!(exists<CapStore<MintCapability<AptosLaunchToken>>>(account_addr), error::not_found(ENO_CAPABILITIES));
        let mint_cap = &borrow_global<CapStore<MintCapability<AptosLaunchToken>>>(account_addr).cap;
        let coins_minted = coin::mint<AptosLaunchToken>(amount, mint_cap);
        coin::deposit<AptosLaunchToken>(dst_addr, coins_minted);
    }
}