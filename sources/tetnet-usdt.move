module testnet_coins::usdt {
    ...
    public entry fun initialize(coin_admin: &signer) {
        ...
        move_to(coin_admin, CapStore<BurnCapability<USDT>> { cap: burn_cap });
        move_to(coin_admin, CapStore<FreezeCapability<USDT>> { cap: freeze_cap });
        move_to(coin_admin, CapStore<MintCapability<USDT>> { cap: mint_cap });
        move_to(coin_admin, Delegations<BurnCapability<USDT>>{ inner: vector::empty<DelegatedCapability<BurnCapability<USDT>>>() });
        move_to(coin_admin, Delegations<FreezeCapability<USDT>>{ inner: vector::empty<DelegatedCapability<FreezeCapability<USDT>>>() });
        move_to(coin_admin, Delegations<MintCapability<USDT>>{ inner: vector::empty<DelegatedCapability<MintCapability<USDT>>>() });
    ...
    public fun destroy_burn_cap_store(cap_store: CapStore<BurnCapability<USDT>>) {
        let CapStore<BurnCapability<USDT>> { cap: cap } = cap_store;
        coin::destroy_burn_cap(cap);
    }
    public fun destroy_freeze_cap_store(cap_store: CapStore<FreezeCapability<USDT>>) {
        let CapStore<FreezeCapability<USDT>> { cap: cap } = cap_store;
        coin::destroy_freeze_cap(cap);
    }
    public fun destroy_mint_cap_store(cap_store: CapStore<MintCapability<USDT>>) {
        let CapStore<MintCapability<USDT>> { cap: cap } = cap_store;
    ...
    public entry fun revoke_burn_capability(sender: &signer, account: address) acquires AdminStore, CapStore {
        ...
        assert!(exists<CapStore<BurnCapability<USDT>>>(account), error::not_found(ENO_CAPABILITIES));
        destroy_burn_cap_store(move_from<CapStore<BurnCapability<USDT>>>(account));
    }
    public entry fun revoke_freeze_capability(sender: &signer, account: address) acquires AdminStore, CapStore {
        only_admin_or_self(sender, account);
        assert!(exists<CapStore<FreezeCapability<USDT>>>(account), error::not_found(ENO_CAPABILITIES));
        destroy_freeze_cap_store(move_from<CapStore<FreezeCapability<USDT>>>(account));
    }
    public entry fun revoke_mint_capability(sender: &signer, account: address) acquires AdminStore, CapStore {
        only_admin_or_self(sender, account);
        assert!(exists<CapStore<MintCapability<USDT>>>(account), error::not_found(ENO_CAPABILITIES));
        destroy_mint_cap_store(move_from<CapStore<MintCapability<USDT>>>(account));
    }
    ...
    public fun burn(
        coin: Coin<USDT>,
        cap_store: &CapStore<BurnCapability<USDT>>
    )   {
        let burn_cap = &cap_store.cap;
        coin::burn<USDT>(coin, burn_cap);
    }

    public fun freeze_account(
        account_to_freeze: address,
        cap_store: &CapStore<FreezeCapability<USDT>>
    )   {
        let freeze_cap = &cap_store.cap;
        coin::freeze_coin_store<USDT>(account_to_freeze, freeze_cap);
    }

    public fun mint(
        amount: u64,
        cap_store: &CapStore<MintCapability<USDT>>
    ): Coin<USDT>   {
        let mint_cap = &cap_store.cap;
        coin::mint<USDT>(amount, mint_cap)
    }
    ...
    public entry fun burn_entry(
        ...
        let account_addr = signer::address_of(account);
        assert!(exists<CapStore<BurnCapability<USDT>>>(account_addr), error::not_found(ENO_CAPABILITIES));
        let burn_cap = &borrow_global<CapStore<BurnCapability<USDT>>>(account_addr).cap;
        coin::burn_from<USDT>(account_addr, amount, burn_cap);
    }
    ...
    public entry fun freeze_entry(
        ...
        let account_addr = signer::address_of(account);
        assert!(exists<CapStore<FreezeCapability<USDT>>>(account_addr), error::not_found(ENO_CAPABILITIES));
        let freeze_cap = &borrow_global<CapStore<FreezeCapability<USDT>>>(account_addr).cap;
        coin::freeze_coin_store<USDT>(account_to_freeze, freeze_cap);
    }
    ...
    public entry fun mint_entry(
        ...
        let account_addr = signer::address_of(account);
        assert!(exists<CapStore<MintCapability<USDT>>>(account_addr), error::not_found(ENO_CAPABILITIES));
        let mint_cap = &borrow_global<CapStore<MintCapability<USDT>>>(account_addr).cap;
        let coins_minted = coin::mint<USDT>(amount, mint_cap);
        coin::deposit<USDT>(dst_addr, coins_minted);
    }
}